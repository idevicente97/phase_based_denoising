...
# **********************************************************************
# This script takes 4 arguments: Subject ID, RUN, PRJDIR path, BASEDIR path
# **********************************************************************
...
if [[ -z "${BASEDIR}" ]]; then
  if [[ ! -z "$4" ]]; then
     ...
     echo -e "\e[31m++ ERROR: You need to input PROJECT DIRECTORY (PRJDIR) as ENVIRONMENT VARIABLE or $4. Exiting!! ...\e[39m"
     exit
  fi
fi
...
ph_steady_state() {
	...
	P_BOLD=`find ${PRJDIR}/BIDS_selected_subjects/$SUBJ/func/* -name "*task-petitPhase*run-${RUN}*bold.nii.gz" -o -name "*task-petit_rec-phase*run-${RUN}*bold.nii.gz"`
	nBold=$(echo "$P_BOLD" | tr " " "\n" | grep -c ".nii.gz")
	P_JSON_FILE=`find ${PRJDIR}/BIDS_selected_subjects/$SUBJ/func/* -name "*task-petitPhase*run-${RUN}*bold.json" -o -name "*task-petit_rec-phase*run-${RUN}*bold.json"`
