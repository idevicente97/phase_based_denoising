...
# Scripts freesurfer must be run before
# This function takes 2 arguments: PRJDIR path, Subject name
# ****************************************************************************
...
do
    ...
    3dNwarpApply -overwrite -master ${SUBJ}_T1_ns.${TSPACE}.nii.gz -dxyz ${VOXELRESZ} \
             -source ${SUBJ}.${GLM_TYPE}.stats.nii.gz                         \
             -nwarp "anat.un.aff.qw_WARP.nii.gz mat.warp.aff12.1D"     \
             -prefix ${SUBJ}.${GLM_TYPE}.stats.${TSPACE}.nii.gz
    ...
    3dBlurInMask -overwrite -float -preserve -FWHM ${FWHM} -mask ${SUBJ}_mask_base.FUNC.${TSPACE}.nii.gz \
             -prefix rm.${SUBJ}.${GLM_TYPE}.stats.blur.${TSPACE}.nii.gz ${SUBJ}.${GLM_TYPE}.stats.${TSPACE}.nii.gz
    3dcalc -overwrite -a rm.${SUBJ}.${GLM_TYPE}.stats.blur.${TSPACE}.nii.gz -m ${SUBJ}_mask_base.FUNC.${TSPACE}.nii.gz \
             -expr 'a*m' -prefix ${SUBJ}.${GLM_TYPE}.stats.blur.${TSPACE}.nii.gz
...
for GLM_TYPE in GLM_mag_REML.tCompCor GLM_odr_REML.tCompCor GLM_lsqrs_REML.tCompCor
do
    echo -e "\e[32m ++ INFO: Warping ${GLM_TYPE} to ${TSPACE} ...\e[39m"
    3dNwarpApply -overwrite -master ${SUBJ}_T1_ns.${TSPACE}.nii.gz -dxyz ${VOXELRESZ} \
             -source ${SUBJ}.${GLM_TYPE}.stats.nii.gz                         \
...
for GLM_TYPE in GLM_mag_REML.HighCor GLM_odr_REML.HighCor GLM_lsqrs_REML.HighCor
do
    echo -e "\e[32m ++ INFO: Warping ${GLM_TYPE} to ${TSPACE} ...\e[39m"
    3dNwarpApply -overwrite -master ${SUBJ}_T1_ns.${TSPACE}.nii.gz -dxyz ${VOXELRESZ} \
             -source ${SUBJ}.${GLM_TYPE}.stats.nii.gz                         \
...
# Warping as well ODR R2 maps for analytical purposes

VOXELRESZ=$(3dinfo -adk ${SUBJ}.odr_r2.nii.gz) #MINE

3dcalc -a ${SUBJ}.odr_r2.nii.gz -m ${SUBJ}_mask_base.nii.gz -expr 'm*abs(a)' -prefix ${SUBJ}.odr_r2_abs_nothr.nii.gz -overwrite

3dNwarpApply -overwrite -master ${SUBJ}_T1_ns.${TSPACE}.nii.gz -dxyz ${VOXELRESZ} -source ${PRJDIR}/PREPROC/${SUBJ}/func/Resp/${SUBJ}.odr_r2_abs_nothr.nii.gz -nwarp "anat.un.aff.qw_WARP.nii.gz mat.warp.aff12.1D" -prefix ${SUBJ}.odr_r2_abs_nothr.${TSPACE}.nii.gz

3dBlurInMask -overwrite -float -preserve -FWHM ${FWHM} -mask ${SUBJ}_mask_base.FUNC.${TSPACE}.nii.gz \
             -prefix rm.${SUBJ}.odr_r2_abs_nothr.blur.${TSPACE}.nii.gz ${SUBJ}.odr_r2_abs_nothr.${TSPACE}.nii.gz

3dcalc -overwrite -a rm.${SUBJ}.odr_r2_abs_nothr.blur.${TSPACE}.nii.gz -m ${SUBJ}_mask_base.FUNC.${TSPACE}.nii.gz \
             -expr 'a*m' -prefix ${SUBJ}.odr_r2_abs_nothr.blur.${TSPACE}.nii.gz
