...
# ***************************************************************************************
# This script takes 5 arguments: Subject ID, PRJDIR path, ORDER of onset times, CENSOR type and MOTION threshold
# ***************************************************************************************
...
if [[ -z "${ORDER}" ]]; then
  if [[ ! -z "$3" ]]; then
     ...
     echo -e "\e[31m++ ERROR: You need to input ORDER as ENVIRONMENT VARIABLE or $3. Exiting!! ...\e[39m"
     exit
  fi
fi
...
ONSETDIR=${PRJDIR}/${SUBJ}/onset_times
...
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Pres.1D.txt -expr 'a' > onset_${SUBJ}_task-petit_${ORDER}_Pres_Raw.1D.txt
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Pres.1D.txt -expr 'a+374' > onset_${SUBJ}_task-petit_${ORDER}_Pres_ODR.1D.txt
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Pres.1D.txt -expr 'a+748' > onset_${SUBJ}_task-petit_${ORDER}_Pres_LSQ.1D.txt

1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Hash.1D.txt -expr 'a' > onset_${SUBJ}_task-petit_${ORDER}_Hash_Raw.1D.txt
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Hash.1D.txt -expr 'a+374' > onset_${SUBJ}_task-petit_${ORDER}_Hash_ODR.1D.txt
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Hash.1D.txt -expr 'a+748' > onset_${SUBJ}_task-petit_${ORDER}_Hash_LSQ.1D.txt

1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Star.1D.txt -expr 'a' > onset_${SUBJ}_task-petit_${ORDER}_Star_Raw.1D.txt
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Star.1D.txt -expr 'a+374' > onset_${SUBJ}_task-petit_${ORDER}_Star_ODR.1D.txt
1deval -overwrite -a onset_${SUBJ}_task-petit_${ORDER}_Star.1D.txt -expr 'a+748' > onset_${SUBJ}_task-petit_${ORDER}_Star_LSQ.1D.txt

1dMarry -divorce onset_${SUBJ}_task-petit_${ORDER}_Scramble.1D.txt > rm.Scramble_divorce1.1D rm.Scramble_divorce2.1D
...
1dMarry -sep ':' rm.Scramble_divorce_A_Raw.1D.txt rm.Scramble_divorce2.1D_B.1D > onset_${SUBJ}_task-petit_${ORDER}_Scramble_Raw.1D.txt
1dMarry -sep ':' rm.Scramble_divorce_A_ODR.1D.txt rm.Scramble_divorce2.1D_B.1D > onset_${SUBJ}_task-petit_${ORDER}_Scramble_ODR.1D.txt
1dMarry -sep ':' rm.Scramble_divorce_A_LSQ.1D.txt rm.Scramble_divorce2.1D_B.1D > onset_${SUBJ}_task-petit_${ORDER}_Scramble_LSQ.1D.txt

#rm rm*
#rm onset_${SUBJ}_task-petit_${ORDER}_Scramble.1D.txt_*


1dMarry -divorce onset_${SUBJ}_task-petit_${ORDER}_Sentence.1D.txt > rm.Sentence_divorce1.1D rm.Sentence_divorce2.1D
...
1dMarry -sep ':' rm.Sentence_divorce_A_Raw.1D.txt rm.Sentence_divorce2.1D_B.1D > onset_${SUBJ}_task-petit_${ORDER}_Sentence_Raw.1D.txt
1dMarry -sep ':' rm.Sentence_divorce_A_ODR.1D.txt rm.Sentence_divorce2.1D_B.1D > onset_${SUBJ}_task-petit_${ORDER}_Sentence_ODR.1D.txt
1dMarry -sep ':' rm.Sentence_divorce_A_LSQ.1D.txt rm.Sentence_divorce2.1D_B.1D > onset_${SUBJ}_task-petit_${ORDER}_Sentence_LSQ.1D.txt

#rm rm*
#rm onset_${SUBJ}_task-petit_${ORDER}_Sentence.1D.txt_*
...
rm ROIPC.tCompCor_concatenated.1D
cat ROIPC.tCompCor.1D ROIPC.tCompCor.1D ROIPC.tCompCor.1D > ROIPC.tCompCor_concatenated.1D
...
STIM_TIMES=${ONSETDIR}/onset_${SUBJ}_task-petit_${ORDER}
...
3dDeconvolve -overwrite -input ${SUBJ}.magnitude.pb03_volreg_detrended_masked_scaled_mean.nii.gz ${SUBJ}.odr_substracted_scaled_mean.nii.gz ${SUBJ}.Lsqrs.magnitude_phdenoised_scaled_mean.nii.gz						\
	...
	-ortvec ROIPC.tCompCor_concatenated.1D PC.tCompCor_concatenated.1D									\
	-polort ${POLORTORDER} -num_stimts 27	                                              							\
	-stim_times_AM1 1 ${STIM_TIMES}_Sentence_Raw.1D.txt 'dmUBLOCK' -stim_label 1 Sentence_Raw 	\
	-stim_times_AM1 2 ${STIM_TIMES}_Scramble_Raw.1D.txt 'dmUBLOCK' -stim_label 2 Scramble_Raw 	\
	...
	-stim_times_AM1 6 ${STIM_TIMES}_Sentence_ODR.1D.txt 'dmUBLOCK' -stim_label 6 Sentence_ODR 	\
	-stim_times_AM1 7 ${STIM_TIMES}_Scramble_ODR.1D.txt 'dmUBLOCK' -stim_label 7 Scramble_ODR 	\
	...
	-stim_times_AM1 11 ${STIM_TIMES}_Sentence_LSQ.1D.txt 'dmUBLOCK' -stim_label 11 Sentence_LSQ 	\
	-stim_times_AM1 12 ${STIM_TIMES}_Scramble_LSQ.1D.txt 'dmUBLOCK' -stim_label 12 Scramble_LSQ 	\
	...
	-fout -tout																\
	-x1D ${SUBJ}.GLM_4MEMA.tCompCor.X.xmat.1D -xjpeg ${SUBJ}.GLM_4MEMA.tCompCor.X.jpg -x1D_uncensored ${SUBJ}.GLM_4MEMA.tCompCor.X.nocensor.xmat.1D				\
	-fitts ${SUBJ}.GLM_4MEMA.tCompCor.fitts.nii.gz -errts ${SUBJ}.GLM_4MEMA.tCompCor.errts.nii.gz -bucket ${SUBJ}.GLM_4MEMA.stats.nii.gz


3dREMLfit -overwrite -matrix ${SUBJ}.GLM_4MEMA.tCompCor.X.xmat.1D \
	...
	-fout -tout -Rbuck ${SUBJ}.GLM_4MEMA_REML.tCompCor.stats.nii.gz -Rvar ${SUBJ}.GLM_4MEMA_REMLvar.tCompCor.stats.nii.gz \
	-Rerrts ${SUBJ}.GLM_4MEMA_REML.tCompCor.errts.nii.gz -Rfitts ${SUBJ}.GLM_4MEMA_REML.tCompCor.fitts.nii.gz -verb
